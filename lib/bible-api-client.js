...
module.exports = class BibleApiClient {
  ...
  #chooseVersion(version) {
    function isNullUndefinedOrWhiteSpaces(str) {
      return str === undefined || str === null || str.match(/^ *$/) !== null;
    }

    const result = isNullUndefinedOrWhiteSpaces(version) ? this.#defaultVersion : version
  ...
  async #findVersesFromGroup(group) {
    ...
    const version = this.#chooseVersion(result.version)
    ...
    if (result.chapter >= 1 && result.chapter <= bookChapterCount) {
      if (result.fromVerse && result.toVerse) {
        ...
        if (response.error) {
          result.error = response.error
        } else if (response.verses && this.#isIterable(response.verses)) {
